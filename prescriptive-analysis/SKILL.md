...
- **Hunt for rival bases before filling value cells.** When a cell's evidence rests on a conversion model or reference cohort computed *for this analysis* (rather than inherited from an upstream skill), the auxiliary-basis discipline applies here too: actively check the warehouse for maintained rate/forecast/LTV model tables before concluding no rival exists, and fork the cell if rivals disagree.
...
- **Beyond-the-scaffold pass (required).** After the required artifacts, take one deliberate unscripted pass — for this skill: *"what does the evidence hint at that no criterion asked about — an option nobody named, a constraint nobody stated?"* — and log its cuts and checks in the same ledger as everything else. Curiosity is made an artifact here because nothing else in this repo survives contact with a checklist.
